...
def _has(actions: set[str], wanted: str) -> bool:
    wanted = wanted.lower()
    return any(fnmatchcase(wanted, granted) or fnmatchcase(granted, wanted) for granted in actions)
...
def analyze(policy: dict[str, Any]) -> list[Finding]:
    ...
    if _has(actions, "*") or _has(actions, "iam:*"):
        findings.append(Finding(
            "IAM001", "critical", 100, "Wildcard administrative permissions",
            tuple(sorted(actions)), "Replace wildcards with task-scoped actions and resources.",
        ))

    compute = any(_has(actions, action) for action in (
        "lambda:CreateFunction", "lambda:UpdateFunctionCode", "ec2:RunInstances", "ecs:RunTask"
    ))
    if _has(actions, "iam:PassRole") and compute:
        findings.append(Finding(
            "IAM002", "critical", 90, "PassRole plus compute execution",
            ("iam:PassRole", "compute-create/run"),
            "Scope PassRole to approved role ARNs and restrict compute execution paths.",
        ))

    if _has(actions, "iam:CreatePolicyVersion") and _has(actions, "iam:SetDefaultPolicyVersion"):
        findings.append(Finding(
            "IAM003", "critical", 90, "Managed-policy version escalation",
            ("iam:CreatePolicyVersion", "iam:SetDefaultPolicyVersion"),
            "Do not grant both policy-version mutation permissions to workload identities.",
        ))

    attach = any(_has(actions, action) for action in (
        "iam:AttachUserPolicy", "iam:AttachRolePolicy", "iam:PutUserPolicy", "iam:PutRolePolicy"
    ))
    if attach and "*" in resources:
        findings.append(Finding(
            "IAM004", "high", 75, "Unscoped policy attachment or inline-policy mutation",
            tuple(sorted(a for a in actions if a.startswith("iam:"))),
            "Scope identity mutation to named roles/users and control which policies can be attached.",
        ))

    if _has(actions, "iam:CreateAccessKey") and "*" in resources:
        findings.append(Finding(
            "IAM005", "high", 70, "Access-key creation is not identity-scoped",
    ...
    if _has(actions, "sts:AssumeRole") and "*" in resources:
        findings.append(Finding(
            "IAM006", "high", 70, "Wildcard role assumption",
