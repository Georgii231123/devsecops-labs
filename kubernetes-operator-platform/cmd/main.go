...
func main() {
	...
	must(platformv1alpha1.AddToScheme(scheme))
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElection:         true,
		LeaderElectionID:       "webservice-operator.platform.example.io",
	})
	must(err)
	must((&controller.WebServiceReconciler{Client: mgr.GetClient(), Scheme: mgr.GetScheme()}).SetupWithManager(mgr))
